...
do
  ...
  ffmpeg -i "$big" -acodec copy -t 00:00:30 -ss $ss_stamp -metadata track="$count/$total_count" -metadata title="$title $zcount" "$prefix-$zcount.mp3" 
  ss=$[$ss+0.5]
  count=$[$count+1]
done
